...
<div class="table-responsive" style="background: #ffffff; border-radius: 15px;
    ...
    <form action="{{ route('Post.update', ['id' => $post->id]) }}" method="POST" enctype="multipart/form-data" style="width: 880px; margin: auto">
        @csrf
        <div class="form-group">
            <label for="title">Title</label>
            <input type="text" class="form-control titlePostAdd" value="{{ $post->title }}" name="titlePostAdd" id="title" onkeyup="ChangeToSlug();" data-validation="length" data-validation-length="min1" data-validation-error-msg="Title không được để trống" placeholder="Enter name">
        </div>
        <div class="form-group">
            <label for="slug">Slug</label>
            <input type="text" class="form-control" value="{{ $post->slug }}" name="SlugPostAdd" id="slug" data-validation="length" data-validation-length="min1" data-validation-error-msg="Slug không được để trống" placeholder="Enter name">
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <input type="text" class="form-control" value="{{ $post->description }}" name="DescriptionPostAdd" id="description" data-validation="length" data-validation-length="min1" data-validation-error-msg="Description không được để trống" placeholder="Enter description">
        </div>
        <div class="form-group">
            <label for="content">Content</label>
            {{-- <textarea id="my_editor" name="PostAdd" id="content" class="form-control"></textarea> --}}
            <textarea name="ContentPostAdd" id="editor1" rows="10" cols="80" data-validation="required" data-validation-error-msg="Content không được để trống">{!! $post->content !!}</textarea>
        </div>
        <div class="form-group">
            <label>Tags</label>
            <select name="PostTagsAdd[]" class="form-control-file" id="select2insidemodal" multiple="multiple"  data-validation="required" data-validation-error-msg="Tags không được để trống">
                @foreach ($tagsForPost as $tagForPost)
                    <option selected="selected" value="{{ $tagForPost->name }}">{{ $tagForPost->name }}</option>
                @endforeach
                @foreach ($tags as $tag)
        ...
        <div class="form-group">
            ...
            <select name="CategoryPostAdd" class="form-control Select2_CategoryPost" id="" data-validation="required" data-validation-error-msg="Category post không được để trống">
                ...
                @foreach ($categoryPosts as $categoryPost)
                    <option {{ $post->categoryPost_id == $categoryPost->id ? 'selected' : '' }} value="{{ $categoryPost->id }}">{{ $categoryPost->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <button class="btn btn-success" style="float: right;" type="submit">Edit</button>
    </form>
</div>
@endsection
